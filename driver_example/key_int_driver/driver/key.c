...
static int key_release(struct inode *inode,struct file *filp )
{
	...
	if(&dev->flag == 0){
		...
		/* 释放中断和对应的GPIO */
		for(i = 0;i < KEY_NUM;i++){
			free_irq(&dev->irqkeydesc[i].irqnum,dev);
...
static const struct file_operations key_fops = 
{
	...
	.release		= key_release,					//close from device
};
...
static int __init keyirq_init(void)  
{
	...
	for(i = 0;i < KEY_NUM; i++ ){
		...
		retvalue = request_irq(key.irqkeydesc[i].irqnum,key.irqkeydesc[i].handler,IRQF_TRIGGER_FALLING | IRQF_TRIGGER_RISING | IRQF_SHARED,key.irqkeydesc[i].name,&key);
		if(retvalue < 0){
